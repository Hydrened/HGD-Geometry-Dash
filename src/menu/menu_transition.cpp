...
// INIT
MenuTransition::MenuTransition(Game* g, TransitionState s, const std::function<void()>& c) : game(g), state(s), completed(c) {
    ...
    if (transitionDuration == 0) {
        ...
        game->inMenuTransition = false;
        delete this;
        return;
    }

    game->inMenuTransition = true;
